#+title: Curso de series temporales para la asignatura Econometría Aplicada
#+subtitle: Grado en Economía de la UCM
#+author: Marcos Bujosa
#+LANGUAGE: es
# +OPTIONS: toc:nil

# +MACRO: imglnk @@html:<a href="$1"><img src="$2"></a>@@

# ###########
# ESTO DA EL FORMATO FINAL DE LA PÁGINA WEB VÉASE [[https://olmon.gitlab.io/org-themes/]]
#+SETUPFILE: ./css/simple_inlineUCM.theme  
# ###########

# +attr_html: :width 320px
# file:./3-2016-07-21-Marca_UCM_logo_negro.png
# https://www.ucm.es/data/cont/docs/3-2016-07-21-Marca%20UCM%20logo%20negro.png

* Enlaces

- Enlaces :: 
  - [[https://mbujosab.github.io/org-lessons-gitpages/][Web]] con el material
  - [[https://github.com/mbujosab/org-lessons-gitpages][Repositorio]] con el material
  - [[https://drive.google.com/open?id=0Bwa1A30pN0IwWUtNNzRQRFRJUTQ][Ficha]] de la asignatura

[[file:Calendario-Econometria-Aplicada.pdf][Calendario tentativo]] 

[[file:sitemap.org][mapa del sitio]]

* Lecciones

** Transformación de los datos (Lecc01)
    :PROPERTIES:  
    :UNNUMBERED: nil
    :END:  

- ([[file:./Transparencias/Lecc01.slides.html][slides]]) --- ([[file:./Lecciones/Lecc01.html][html]]) --- ([[file:./Lecciones/Lecc01.pdf][pdf]]) --- ([[https://mybinder.org/v2/gh/mbujosab/org-lessons-gitpages/gh-pages?labpath=CuadernosElectronicos/Lecc01.ipynb][lección en mybinder]])

# - [[https://mybinder.org/v2/gh/mbujosab/EconometriaAplicada-SRC/HEAD?labpath=Lecc01.ipynb][notebook en mybinder]]

- [[https://mybinder.org/v2/gh/mbujosab/org-lessons-gitpages/gh-pages?urlpath=%2Fdoc%2Ftree%2Fdocs%2FCuadernosElectronicos%2FLecc01.ipynb][notebook en mybinder]]

*** Practicas

- ([[file:./Lecciones/Practicas/Prct-Lecc01.html][html]]) --- ([[file:./Lecciones/Practicas/Prct-Lecc01.pdf][pdf]])
- Guiones de gretl: 
  + [[file:./Lecciones/Practicas/guiones/lecc01-airlinePass.inp][lecc01-airlinePass.inp]]


** Conceptos algebraicos (Lecc02)
    :PROPERTIES:  
    :UNNUMBERED: nil
    :END:  

- ([[file:./Transparencias/Lecc02.slides.html][slides]]) --- ([[file:./Lecciones/Lecc02.html][html]]) --- ([[file:./Lecciones/Lecc02.pdf][pdf]]) --- ([[https://mybinder.org/v2/gh/mbujosab/org-lessons-gitpages/gh-pages?labpath=CuadernosElectronicos/Lecc02.ipynb][lección en mybinder]])
  + [[https://mybinder.org/v2/gh/mbujosab/org-lessons-gitpages/gh-pages?labpath=CuadernosElectronicos/implementacion_series_formales.ipynb][implementacion_series_formales]] 

# - [[https://mybinder.org/v2/gh/mbujosab/EconometriaAplicada-SRC/HEAD?labpath=Lecc04.ipynb][notebook en mybinder]]

- [[https://mybinder.org/v2/gh/mbujosab/org-lessons-gitpages/gh-pages?labpath=%2Fdoc%2Ftree%2Fdocs%2FCuadernosElectronicos%2FLecc02.ipynb][notebook en mybinder]]


** Procesos estocásticos débilmente estacionarios (Lecc03)

- [[file:./Lecc05.slides.html][slides]]
- [[file:./Lecc05.html][html]]
- [[file:./Lecc05.pdf][pdf]]
- [[https://mybinder.org/v2/gh/mbujosab/EconometriaAplicada-SRC/HEAD?labpath=Lecc05.ipynb][notebook en mybinder]]
  

** ACF, PACF y densidad espectral de algunos modelos lineales (Lecc04)

- [[file:./Lecc06.slides.html][slides]]
- [[file:./Lecc06.html][html]]
- [[file:./Lecc06.pdf][pdf]]
- [[https://mybinder.org/v2/gh/mbujosab/EconometriaAplicada-SRC/HEAD?labpath=Lecc06.ipynb][notebook en mybinder]]


** ACF, PACF y densidad espectral de algunos modelos ARMA. Herramientas estadísticas (Lecc05)

- [[file:./Lecc07.slides.html][slides]]
- [[file:./Lecc07.html][html]]
- [[file:./Lecc07.pdf][pdf]]
- [[https://mybinder.org/v2/gh/mbujosab/EconometriaAplicada-SRC/HEAD?labpath=Lecc07.ipynb][notebook en mybinder]]


** Modelos ARIMA y SARIMA. Identificación y diagnosis del análisis univariante (Lecc06)

- [[file:./Lecc08.slides.html][slides]]
- [[file:./Lecc08.html][html]]
- [[file:./Lecc08.pdf][pdf]]
- [[https://mybinder.org/v2/gh/mbujosab/EconometriaAplicada-SRC/HEAD?labpath=Lecc08.ipynb][notebook en mybinder]]



** Regresión con series temporales - Modelos con componentes deterministas (Lecc07) :noexport:

- [[file:./Lecc02.slides.html][slides]]
- [[file:./Lecc02.html][html]]
- [[file:./Lecc02.pdf][pdf]]
- [[https://mybinder.org/v2/gh/mbujosab/EconometriaAplicada-SRC/HEAD?labpath=Lecc02.ipynb][notebook en mybinder]]

**** Practicas
:PROPERTIES:
:ID:       ec0682a8-7d04-4bbf-84f9-2e4c78db7b17
:END:

- [[file:./Practicas/PracticasGretl/pub/Prct-Lecc02.pdf][pdf file]]
- Guiones de gretl
  + [[file:./Practicas/PracticasGretl/pub/scripts/componentesAirlinePass.inp][componentesAirlinePass.inp]]
  + [[file:./Practicas/PracticasGretl/pub/scripts/componentesAirlinePass2.inp][componentesAirlinePass2.inp]]

[[file:sitemap.org][mapa del sitio]]
